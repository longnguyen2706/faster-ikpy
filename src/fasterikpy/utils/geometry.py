...
def rx_matrix(theta):
    ...
    return cp.array([
        [1, 0, 0],
        [0, cp.cos(theta), -cp.sin(theta)],
        [0, cp.sin(theta), cp.cos(theta)]
    ])
...
def rotation_matrix(phi, theta, psi):
    ...
    return cp.dot(rz_matrix(phi), cp.dot(rx_matrix(theta), rz_matrix(psi)))
...
def rpy_matrix(roll, pitch, yaw):
    ...
    return cp.dot(rz_matrix(yaw), cp.dot(ry_matrix(pitch), rx_matrix(roll)))
...
def axis_rotation_matrix(axis, theta):
    ...
    c = cp.cos(theta)
    s = cp.sin(theta)
    return cp.array(_axis_rotation_matrix_formula(x, y, z, c, s))
...
def homogeneous_translation_matrix(trans_x, trans_y, trans_z):
    ...
    return cp.array([cp.array([1, 0, 0, trans_x]), cp.array([0, 1, 0, trans_y]), cp.array([0, 0, 1, trans_z]), cp.array([0, 0, 0, 1])])
...
def to_transformation_matrix(translation, orientation_matrix=cp.zeros((3, 3))):
...
def cartesian_to_homogeneous(cartesian_matrix, matrix_type="numpy"):
    ...
    if matrix_type == "numpy":
        homogeneous_matrix = cp.eye(dimension_x + 1)
...
def cartesian_to_homogeneous_vectors(cartesian_vector, matrix_type="numpy"):
    ...
    # Vector
    if matrix_type == "numpy":
        homogeneous_vector = cp.zeros(dimension_x + 1)
        # Last item is a 1
